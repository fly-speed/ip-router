...
INSTALL_ROOT=${IP_ROUTER_HOME:-/opt/soft/ip-router}
...
require_master() {
	if [ ! -x "$MASTER_CTL" ]; then
		...
		echo "Install acl-master under $MASTER_ROOT before installing ip-router." >&2
		exit 1
	fi
...
unregister_service() {
	...
	temporary="$MASTER_SERVICES.ip-router.$$"
